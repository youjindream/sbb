...
#Grid 레이아웃
`row` : 가로 한 줄 영역을 만드는 클래스
`col` : row 안에서 열 영역을 만드는 클래스(한 줄을 여러 칸으로 나누기)
`col-6` : `row`를 기본 12칸으로 봤을 때 그중 6칸, 즉 절반 너비를 차지하는 열 클래스.
...
`input-group` : 입력칸과 버튼·텍스트 등을 하나의 그룹으로 묶어 붙여 배치하는 클래스.
